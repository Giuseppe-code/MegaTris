...
public class helpActivity3 extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        ImageButton buttonn = findViewById(R.id.next);
        ImageButton pre = findViewById(R.id.previous);
        TextView t =findViewById(R.id.col2);


//textview
        Spannable word = new SpannableString("in ");
        word.setSpan(new ForegroundColorSpan(Color.BLACK), 0, word.length(), Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
        t.setText(word);
        Spannable wordTwo = new SpannableString("each section ");
        wordTwo.setSpan(new ForegroundColorSpan(Color.rgb(0,169,157)), 0, wordTwo.length(), Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
        t.append(wordTwo);
        Spannable wordTre = new SpannableString("of the ");
        wordTre.setSpan(new ForegroundColorSpan(Color.BLACK), 0, wordTre.length(), Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
        t.append(wordTre);
        Spannable wordfor = new SpannableString("table ");
        wordfor.setSpan(new ForegroundColorSpan(Color.BLACK), 0, wordfor.length(), Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
        t.append(wordfor);
        Spannable wordfive = new SpannableString("there's ");
        wordfive.setSpan(new ForegroundColorSpan(Color.BLACK), 0, wordfive.length(), Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
        t.append(wordfive);
        Spannable wordsix = new SpannableString("another table ");
        wordsix.setSpan(new ForegroundColorSpan(Color.rgb(0,169,157)), 0, wordsix.length(), Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
        t.append(wordsix);

        pre.setOnClickListener((View view) -> {
            Intent intent = new Intent(helpActivity3.this, helpActivity2.class);
        ...
        buttonn.setOnClickListener((View view)->{
            Intent intent = new Intent(helpActivity3.this,helpActivity4.class);
            startActivity(intent);
            overridePendingTransition(android.R.anim.fade_in, android.R.anim.fade_out);
        });
